...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16, horizon_lines=None):
    ...
    for i, img in enumerate(images):
        ...
        # ==== 在缩放后、放入马赛克前绘制水平线 ====
        if horizon_lines is not None and i < len(horizon_lines):
            y_norm = horizon_lines[i]  # 归一化坐标 [0,1]
            y_pixel = int(y_norm * h)  # 转换为实际像素

            # 绘制水平线（绿色，2像素宽）
            cv2.line(img, (0, y_pixel), (w, y_pixel), (0, 255, 0), 2)

            # 添加文字标注
            label = f'Horo: {y_norm:.2f}'
            cv2.putText(img, label, (10, y_pixel - 10),
        ...
        if horizon_lines is not None:
            for i in range(len(images)):
                if i >= len(horizon_lines):
                    continue
                img = images[i]
                h, w = img.shape[:2]
                y = int(horizon_lines[i] * h)  # 将归一化坐标转换为实际像素

                # 绘制水平线（绿色，2像素宽）
                cv2.line(img, (0, y), (w, y), (0, 255, 0), 2)

                # 添加文字标注
                label = f'Horizon: {horizon_lines[i]:.3f}'
